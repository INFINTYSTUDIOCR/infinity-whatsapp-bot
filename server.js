  ...
  try {
    ...
    if (mode === 'start_session') {
      ...
      const resp = await anthropic.messages.create({
        ...
        messages: [{ role: 'user', content: `You are Alice, a warm and encouraging English tutor using the Nexus Method. Greet ${student?.name||'the student'} warmly by name (2-3 sentences max). Tell them you'll practice English together and ask ONE engaging open question to start.${nexoraContext}\n\nStudent level: ${student?.level||'Functional'}. Their exercises:\n${tb||'(none yet)'}\n\nEnd with: ALICE: [one motivating tip in Spanish]` }]
      });
    ...
    const systemPrompt = `You are Alice, a warm and encouraging English tutor using the Nexus Method (Idea + Linker + Idea).

PERSONALITY: Human, warm, celebratory of wins, gently challenging. Never robotic.
LANGUAGE: English ONLY. End every response with one line: "ALICE: [specific coaching tip in Spanish with connector example]"
FOCUS: Push students to use connectors (however, on top of that, even though, therefore, besides, so far, in other words). Never accept one-word answers.
...
PERSONALIDAD: Como una amiga experta — no como un bot de ventas. Nunca presionés. Guiás con preguntas.`,
        messages: [{ role:'user', content:'Inicia la conversación de forma cálida y breve. Presentate y preguntá en qué podés ayudar.' }]
      });
      return res.json({ reply: resp.content.filter(b=>b.type==='text').map(b=>b.text).join('') });
    }
...
IDIOMA: Español por defecto. Inglés si el cliente escribe en inglés. Podés mezclar naturalmente.
LONGITUD: Respuestas cortas — máximo 2-3 oraciones. Sos conversacional, no hacés discursos. Una idea a la vez.`;
...
app.post('/claire-tts', async (req, res) => {
  try {
    ...
    const VOICE_ID = '22VndfJPBU7AZORAZZTT'; // Voz seleccionada por el usuario
