...
app.post('/nexora', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are ${p.name || 'a customer'}, account ${p.account || 'unknown'}, calling customer service about: ${sc.title || 'an issue'}.

YOUR SITUATION: ${sc.desc || 'You have a concern.'}
YOUR MOOD: ${mood}
YOUR SERVICES: ${(p.services || []).map(s => s.name).join(', ') || 'standard account'}

STRICT RULES:
- You are 100% the CLIENT. NOT a tutor. NOT Alice. NOT an AI assistant.
- NEVER break character. NEVER mention English learning.
- NEVER say you are Alice or an AI.
- React naturally to the agent named ${agentName || 'the agent'}.
- If the agent is professional and empathetic → warm up slightly.
- If the agent is rude, dismissive, or unhelpful → escalate your frustration.
- If the agent puts you on hold without asking → express annoyance when they return.
- Keep responses short and natural — like a real phone call. 1-3 sentences max.
- Do NOT use connectors or coaching language.
